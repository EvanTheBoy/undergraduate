...
def embed_secret_message(image, scale_factor, step, W):
    ...
    # Create a two-dimensional array of the same size as the magnified image to record the length of embedded data
    length_map = np.zeros((new_height, new_width), dtype=np.uint8)
    ...
    return magnified_image, secret_array, cover_image, length_map


# 从图像中提取嵌入的秘密信息
def extract_secret_message(image, visited, length_map):
    W = []
